...
def distribute_c_g_r(qc,qg,qr,n4,today):
    ...
    df_nct = pd.read_sql("select * from nc_status",con = con ,index_col = 'index')
    sql = "select * from '"+today+"'"
    df_vmt = pd.read_sql(sql,con = con ,index_col = 'index')
    #global gncid
    ...
    if flag_n == 0 and n4.empty() == False:
        gncid = n4.get()
        flag_n = gncid
    ...
    while  (n4.empty() == False) and (qc.empty() == False):
        if flag_vc == 0:
            ...
            vmcid = flag_vc            
            #print("vmcid = qc.get1:",vmcid)
            
        flag = wt_db_cgr(vmcid,gncid,today)
        if flag == 0:
            gncid = n4.get()
            flag_n = gncid
        else:
    ...
    while  (n4.empty() == False) and (qg.empty() == False):
        if flag_vg == 0:
            ...
            vmgid = qg.get()
            flag_vg = vmgid
            #print("vmgid = qg.get1:",vmgid)
        else:
            vmgid = flag_vg
            
        flag = wt_db_cgr(vmgid,gncid,today)
        if flag == 0:
            gncid = n4.get()
            flag_n = gncid
        else:
...
def distribute_c_r(ncq,vmq,today):
    ...
    global df_vmtd
    con = sqlite3.connect('user.db')
    df_nctd = pd.read_sql("select * from nc_status",con = con ,index_col = 'index')
    sql = "select * from '"+today+"'"
    df_vmtd = pd.read_sql(sql,con = con ,index_col = 'index')
    ...
    df_nctd.to_sql('nc_status',con,if_exists = 'replace')
    df_vmtd.to_sql(today,con,if_exists = 'replace')
...
def wt_db(vmid,ncid,today):
    ...
    vcpu = int(df_vmtd['cpu'][df_vmtd['vmid'] == vmid].values[0])
    vmemory = int(df_vmtd['memory'][df_vmtd['vmid'] == vmid].values[0])
    
    ncpu = int(df_nctd['ncpu'][df_nctd['ncid'] == ncid].values[0])
    nmemory = int(df_nctd['nmemory'][df_nctd['ncid'] == ncid].values[0])
    
    ucpu =  int(df_nctd['ucpu'][df_nctd['ncid'] == ncid].values[0])
    umemory =  int(df_nctd['umemory'][df_nctd['ncid'] == ncid].values[0])
    ...
    if (vcpu + ucpu <= ncpu) and (vmemory +umemory <= nmemory):
        ...
        df_nctd.loc[df_nctd['ncid'] == ncid,'ucpu'] = ucpu
        df_nctd.loc[df_nctd['ncid'] == ncid,'umemory'] = umemory
        
        df_vmtd.loc[df_vmtd['vmid'] == vmid,'n'] = ncid
...
def wt_db_cg(vmid,ncid,today):
    ...
    vcpu = int(df_vmt['cpu'][df_vmt['vmid'] == vmid].values[0])
    vmemory = int(df_vmt['memory'][df_vmt['vmid'] == vmid].values[0])
    
    #ncpu = df_nct['ncpu'][df_nct['ncid'] == ncid].values[0]
    #nmemory = df_nct['nmemory'][df_nct['ncid'] == ncid].values[0]
    
    ucpu =  int(df_nct['ucpu'][df_nct['ncid'] == ncid].values[0])
    umemory =  int(df_nct['umemory'][df_nct['ncid'] == ncid].values[0])
    
    if (vcpu + ucpu <= 96) and (vmemory +umemory <= 128):
        ucpu = ucpu + vcpu
        umemory = umemory + vmemory
        
        
        df_nct.loc[df_nct['ncid'] == ncid,'ucpu'] = ucpu
...
def wt_db_cgx(vmid,ncid,today):
    ...
    if (vcpu + ucpu <= 96) and (vmemory +umemory <= 256):
        ucpu = ucpu + vcpu
        umemory = umemory + vmemory
...
def wt_db_cgr(vmid,ncid,today):
    ...
    ncpu = int(df_nct['ncpu'][df_nct['ncid'] == ncid].values[0])
    nmemory = int(df_nct['nmemory'][df_nct['ncid'] == ncid].values[0])
...
def predict():
    ...
    if c_day < 10:
        ...
        if pdata[2] - 0.5*ln8d[0] > 256:        
            c = int((pdata[2]-0.5*ln8d[0])//516)
        else:
            c = 0
        demand = [a,b,c]
        con.close
